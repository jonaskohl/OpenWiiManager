...
namespace OpenWiiManager.Controls
{
    ...
    public class AeroToolStripRenderer : ToolStripSystemRenderer
    {
        ...
        private static Padding GetThemeMargins(VisualStyleRenderer rnd, IDeviceContext dc = null, MarginProperty prop = MarginProperty.ContentMargins)
        {
            RECT rc = new();
            using (var hdc = new Tools.InteropUtil.SafeDCHandle(dc))
                UxTheme.GetThemeMargins(rnd.Handle, hdc, rnd.Part, rnd.State, (int)prop, IntPtr.Zero, out rc);
            return new Padding(rc.Left, rc.Top, rc.Right, rc.Bottom);
        }

        //private Padding _GetThemeMargins(IDeviceContext dc, MarginType marginType)
        //{
        //    var margins = new MARGIN(0);
        //    try
        //    {
        //        IntPtr hDc = dc.GetHdc();
        //        if (0 == UxTheme.GetThemeMargins(_renderer.Handle, hDc, _renderer.Part, _renderer.State, (int)marginType, IntPtr.Zero, ref margins))
        ...
        protected override void OnRenderImageMargin(ToolStripRenderEventArgs e)
        {
            ...
            if (EnsureRenderer())
            {
                if (e.ToolStrip.IsDropDown)
                {
                    ...
                    Padding margins = GetThemeMargins(_renderer, e.Graphics, MarginProperty.SizingMargins);
